...
namespace ColorMine.ColorSpaces.Conversions
{
	internal static class MunsellConverter
	{
		...
		internal static void ToColorSpace(IRgb color, IMunsell item) {
			if(Math.Round(color.R,0) == Math.Round(color.G,0) && Math.Round(color.G,0) == Math.Round(color.B, 0)) {
		...
		internal static IRgb ToColor(IMunsell item) {
			//明度max or minは白黒で返す
			if (item.V <= 0.0) return new Rgb { R = 0, G = 0, B = 0 };
			if (item.V >= 10.0) return new Rgb { R = 255, G = 255, B = 255 };
			if (item.H.Base == HueBase.N) {
				return ConvertAchromatic(item);
			}

			//2.5刻み値ジャストならマンセル検索→Lab値変換
			if (System.Linq.Enumerable.Range(1, 4).Select(x => x * 2.5).Contains(item.H.Number)) {
				return ConvertLch(item,item.H).To<Rgb>();
			}

			//2.5刻み値以外は両隣の色相から補完値を計算
			var lowH = new MunsellHue(Math.Floor(item.H.Number / 2.5) * 2.5, item.H.Base);
			var highH = new MunsellHue(Math.Ceiling(item.H.Number / 2.5) * 2.5, item.H.Base);
			var rate = (item.H.Number % 2.5) / 2.5;    //2.5の間の割合
			//両隣の色相の補完値同士の補完値を求める
		...
		private static Lch ConvertLch(IMunsell item,MunsellHue hue) {
			...
			if (tmpV == 0.0) tmpV = 1.0;
			if (tmpV == 10.0) tmpV = 9.0;
			//彩度max min
			var c = Table.Where(x => x.H == hue && x.V == tmpV)
				.GroupBy(x => x.V).Select(x => new { min = x.Min(y => y.C), max = x.Max(y => y.C) }).First();
			...
			if (c.min <= item.C && item.C <= c.max) {
				...
				//上記以外は両隣の彩度を基準とする
					...
					var lowC = Math.Floor(item.C / 2) * 2;
					var highC = Math.Ceiling(item.C / 2) * 2;
		...
		private static Lch GetInterpolatedValue(IMunsell item, Lch lowlch, Lch highlch, double rate = 0.5) {
			//彩度の調整
			var diffC = Math.Abs(highlch.C - lowlch.C);
			var newMetricC = (item.C % 2.0) / 2.0 * diffC + Math.Min(lowlch.C, highlch.C);
			//色相の調整
		...
		private static MunsellDat FindMunsell(MunsellHue hue, double value, double chroma,bool retry=false) {
			var rec = Table.Where(x => x.H == hue && x.V == value && x.C == chroma).FirstOrDefault();
